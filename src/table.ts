...
export class Table {
    ...
    private initializeDeck(): void {
        ...
        for (let suit in Suit) {
            if (!isNaN(Number(suit))) continue; // Ignorar las claves numéricas
    ...
    private determineWinners(): void {
        ...
        const winners = this.players.filter(player => !player.isFolded);
        const winner = winners[0]; // Escoger al primero por simplicidad
    ...
    playerAction(playerId: string, action: PlayerAction, amount?: number): void {
        ...
        if (playerId != currentPlayerId) {
            throw new Error(`It's not player ${playerId} turn to play. Is player id: ${currentPlayerId} turn` )
        }
        ...
        switch (action) {
            ...
            case PlayerAction.Call:
                ...
                if (player.chips < callAmount) throw new Error("No hay saldo suficiente para igualar")
            ...
            case PlayerAction.Raise:
                if (amount !== undefined) {
                    ...
                    if (player.chips < raiseAmount) throw new Error("No hay fichas suficientes para Raise")
            ...
            case PlayerAction.Bet:
                ...
                if (amount === undefined) throw new Error("Debe especificar un monto para accion Bet")
                if (amount < this.minimumBet){
                    throw new Error(`El monto minimo es ${this.minimumBet}`)
                }      
                
                if (player.id === this.players[this.currentSmallBlindIndex].id 
                    && amount < this.smallBlind){
                        throw new Error(`Small Blind must bet at least ${this.smallBlind} amount`)
                }
                // Big Blind cant be lower than game big blind
                if (player.id === this.players[this.currentBigBlindIndex].id 
                    && amount < this.bigBlind){
                        throw new Error(`Big Blind must bet at least ${this.bigBlind} amount`)
                }
    ...
    private isBettingRoundOver(): boolean {
        ...
        return this.players.every(player => player.isFolded || player.betAmount === this.currentBet);
    }
